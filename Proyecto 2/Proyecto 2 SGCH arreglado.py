        ...
        while True:
            try:
                ...
                if fila not in "ABCDEFGHIJ":
                    raise ValueError("La fila ingresada no es válida. Debe estar entre A y J.")
                
                columna = int(input(f"{self.nombre}, ingresa la columna para el barco (1-10): ")) - 1
                if not (0 <= columna < self.tablero.dimension):
                    raise ValueError("La columna ingresada está fuera de rango. Debe estar entre 1 y 10.")
